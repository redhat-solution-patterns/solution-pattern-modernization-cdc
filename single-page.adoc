...
3. Next, https://debezium.io[Debezium] streams the data them over to Kafka using https://www.redhat.com/en/resources/amq-streams-datasheet[Red Hat AMQ Streams].
...
The solution is built on top of a hybrid cloud model, with containerized services running on OpenShift (can be on a private or public cloud depending on how you provision the demo) consuming AMQ Streams (Apache Kafka). The Red Hat® AMQ streams component is a massively scalable, distributed, and high-performance data streaming platform based on the Apache Kafka project.
